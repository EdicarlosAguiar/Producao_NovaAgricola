...
public class ConsultaFornecedor extends javax.swing.JDialog {
    ...
    static ArrayList<String> listaFornecedores = new ArrayList();
    ...
    public void buscaFornecedor() {
        ...
        try {
            ...
            pst = conn.getConexao().prepareStatement("select * from fornecedor where razaoSocial LIKE?");
    ...
    public void carregaTabela() {
        ...
        DefaultTableModel modelo = (DefaultTableModel) tabela.getModel();
        modelo.setNumRows(0);
        int totalItem = listaFornecedores.size();
        int i = 0;
        while (totalItem > i) {
            {
                modelo.addRow(new Object[]{
                    tabela.getRowCount() + 1,
                    listaFornecedores.get(i),//Codigo
                    listaFornecedores.get(i + 1),//Categoria
                    listaFornecedores.get(i + 2),//Categoria
                    listaFornecedores.get(i + 3),//Categoria
                }
                );
    ...
    public void carregaArray() {
        ...
        try {
            ...
            ResultSet rs;
            pst = conn.getConexao().prepareStatement("select * from fornecedor");
            rs = pst.executeQuery();
            while (rs.next()) {
                listaFornecedores.add(rs.getString("ID"));
                listaFornecedores.add(rs.getString("razaoSocial"));
                listaFornecedores.add(rs.getString("nomeFantasia"));
                listaFornecedores.add(rs.getString("CNPJ_CPF"));
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ConsultaFornecedor dialog = new ConsultaFornecedor(new javax.swing.JFrame(), true);
